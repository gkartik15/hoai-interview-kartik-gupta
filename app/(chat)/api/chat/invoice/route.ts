...
import { findDuplicateInvoice, saveInvoice, saveMessages, saveTokenUsage } from '@/lib/db/queries';
...
import { calculateStreamingResponseTokenUsage, calculateTokenUsage } from '@/lib/utils/token-tracker';
...
export async function POST(request: Request) {
    try {
        ...
        const validationSystemPrompt = `You are an invoice validation expert. Analyze the provided document text and:
        ...
        const validationUserPrompt = `Analyze this document and return a JSON response with this exact structure:
                {
                    "validation": {
        ...
        const streamingResponseSystemPrompt = `You are an invoice processing assistant. Follow these rules:
        ...
        const streamingResponseUserPrompt = !parsedResponse.validation.isValidInvoice 
        ...
        execute: (dataStream) => {
            const result = streamText({
                ...
                messages: [
                    {
                        role: 'system',
                        content: streamingResponseSystemPrompt
                    },
                    {
                        role: 'user',
                        content: streamingResponseUserPrompt
                    }
                ],
                maxSteps: 5,
                ...
                onFinish: async ({ response, reasoning }) => {
                    ...
                    // Save token usage after successful invoice save and streaming response
                    if (parsedResponse.validation.isValidInvoice && !parsedResponse.validation.isDuplicate) {
                        const validationResponseTokenUsage = calculateTokenUsage(
                            validationSystemPrompt + validationUserPrompt,
                            validationResult.text
                        );
                        const streamingResponseTokenUsage = calculateStreamingResponseTokenUsage(
                            streamingResponseSystemPrompt + streamingResponseUserPrompt
                        );
                        await saveTokenUsage({
                            invoiceId: parsedResponse.validation.invoiceId,
                            usage: {
                                inputTokens: streamingResponseTokenUsage.inputTokens + validationResponseTokenUsage.inputTokens,
                                outputTokens: streamingResponseTokenUsage.outputTokens + validationResponseTokenUsage.outputTokens,
                                totalTokens: streamingResponseTokenUsage.totalTokens + validationResponseTokenUsage.totalTokens,
                                estimatedCost: streamingResponseTokenUsage.estimatedCost + validationResponseTokenUsage.estimatedCost
                            }
                        });
                    }
                },
